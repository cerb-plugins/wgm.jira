...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Macros -->
		{if $is_writeable}
		{devblocks_url assign=return_url full=true}c=profiles&type=jira_project&id={$page_context_id}-{$jira_project->name|devblocks_permalink}{/devblocks_url}
		{include file="devblocks:cerberusweb.core::internal/macros/display/button.tpl" context=$page_context context_id=$page_context_id macro_event="event.macro.jira_project" return_url=$return_url}
		{/if}
		
		<!-- Edit -->
		{if $is_writeable}
		<button type="button" id="btnDisplayJiraProjectEdit" title="{'common.edit'|devblocks_translate|capitalize}"><span class="glyphicons glyphicons-cogwheel"></span></button>
		{/if}
	</form>
...
<fieldset class="properties">
	<legend>{'Jira Project'|devblocks_translate|capitalize}</legend>
...
<div id="profileJiraProjectTabs">
	<ul>
		...
		<li><a href="{devblocks_url}ajax.php?c=profiles&a=handleSectionAction&section=jira_project&action=showIssuesTab&point={$point}&context={$page_context}&context_id={$page_context_id}{/devblocks_url}">{'wgm.jira.common.issues'|devblocks_translate|capitalize}</a></li>
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 101:  // (E) edit
			try {
				$('#btnDisplayJiraProjectEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {
