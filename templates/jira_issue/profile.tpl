...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Edit -->
		<button type="button" id="btnDisplayJiraIssueEdit" title="{'common.edit'|devblocks_translate|capitalize}">&nbsp;<span class="cerb-sprite2 sprite-gear"></span>&nbsp;</button>
	</form>
...
<fieldset class="properties">
	<legend>{'Jira Issue'|devblocks_translate|capitalize}</legend>
...
<fieldset class="properties">
	...
	<div style="margin-left:15px;">
		<pre class="emailbody">{$description|escape:'html'|devblocks_hyperlinks nofilter}</pre>
	</div>
</fieldset>
{/if}
...
<div id="jira_issueTabs">
	<ul>
		...
		<li><a href="{devblocks_url}ajax.php?c=profiles&a=handleSectionAction&section=jira_issue&action=showDiscussionTab&point={$point}&context={$page_context}&context_id={$page_context_id}{/devblocks_url}">{'common.discussion'|devblocks_translate|capitalize}</a></li>
...
<script type="text/javascript">
	$(function() {
		var tabs = $("#jira_issueTabs").tabs( { selected:{$tab_selected_idx} } );
